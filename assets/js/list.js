...
(function(){
  ...
  function render(){
    ...
    if(rp){
      ...
      if(nav2){
        // 大类(cat) 折叠
        nav2.querySelectorAll('.rp-cat').forEach(cat=>{
          const ct=cat.querySelector('.rp-cat-title');
          const ca=cat.querySelector('.rp-arrow');
        ...
        routes.forEach(r=> r.addEventListener('click', ()=>{
          const cat=r.closest('.rp-cat');
          if(cat && !cat.classList.contains('open')){ cat.classList.add('open'); const ca=cat.querySelector('.rp-arrow'); if(ca) ca.textContent='▼'; }
