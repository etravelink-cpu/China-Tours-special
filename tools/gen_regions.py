...
def rid(region, name, code):
    ...
    r=base; i=1
    while r in seen: i+=1; r=base+str(i)
    seen[r]=True; return r
...
def price_table(adult, cno, cw, single, other):
    items=[('成人报名费 Tour Fee','Adult',adult),('儿童不占床 Child no bed','Child no bed',cno),
           ('儿童占床 Child with bed','Child with bed',cw)]
    if single not in ('',None): items.append(('单房差 Single suppl.','Single',single))
    if other not in ('',None): items.append(('综合服务费 Service fee','Service',other))
    body=''.join('<tr><td class="item">'+z+'<span class="en">'+en+'</span></td><td class="price">'+fmt_price(p)+'</td></tr>' for z,en,p in items)
    return '<table class="rp-pricetable"><thead><tr><th>项目</th><th style="text-align:right">价格 (AUD)</th></tr></thead><tbody>'+body+'</tbody></table>'

def pane(region, item, idx):
    name=item['产品名称name'] or ''
    code=item.get('TourCode') or ''
    ...
    '        <div class="rp-summary"><div><b>行程天数</b>'+days+'</div><div><b>抵达城市</b>'+str(item.get('抵达城市arr') or '待确认')+'</div><div><b>离开城市</b>'+str(item.get('出发城市dep') or '待确认')+'</div></div>\n'
...
def region_of(name):
    n=str(name)
    rules=[('新疆','新疆'),('喀纳斯','新疆'),('伊犁','新疆'),('丝路','新疆'),
           ('江南','江南'),('上海','华东'),('杭州','华东'),('苏州','华东'),('南京','华东'),
           ('北京','华北'),('西安','华北'),('中原','华北'),('山西','华北'),
           ('九寨','西南'),('成都','西南'),('重庆','西南'),('西藏','西南'),('川藏','西南'),('长江','三峡'),('三峡','三峡'),
           ('东北','东北'),('哈尔滨','东北'),('雪乡','东北'),('长春','东北'),('沈阳','东北'),('吉林','东北'),
           ('云南','云贵桂'),('贵州','云贵桂'),('桂林','云贵桂'),('广深','大湾区'),('广州','大湾区'),('大湾区','大湾区'),
           ('台湾','台湾'),('厦门','东南'),('福建','东南'),('潮汕','东南')]
    for k,v in rules:
        if k in n: return v
    return '其他'
...
def build_block(region, items):
    cats=OrderedDict()
    CAT_ORDER=['超值特价','纯玩无购物','机票套餐·特别定制']
    for it in items:
        typ=it.get('类型type') or it.get('分组group') or '超值特价'
        if typ not in CAT_ORDER: typ='超值特价'
        cats.setdefault(typ, OrderedDict())
        rg=region_of(it.get('产品名称name') or '')
        cats[typ].setdefault(rg, []).append(it)
    nav=''; panes=''; idx=0
    for cat in CAT_ORDER:
        if cat not in cats: continue
        cat_routes=''
        for rg, its in cats[cat].items():
            routes=''.join('        <div class="rp-route" data-route="'+rid(region,(x.get('产品名称name') or ''),x.get('TourCode') or '')+'">'+(x.get('产品名称name') or '')+'</div>\n' for x in its)
            cat_routes+=('        <div class="rp-group" data-group="'+rg+'">\n          <div class="rp-group-title">'+rg+' <span class="rp-arrow">▶</span></div>\n          <div class="rp-group-list">\n'+routes+'          </div>\n        </div>\n')
    ...
    '  <p class="rp-banner-credit" style="color:#999;font-size:12px;margin:14px 0 0 0;">'+CREDIT+'</p>`;')

# 按板块分组
by_region=OrderedDict()
for it in rows:
    rg=it.get('板块region') or ''
    if rg in MANAGED: by_region.setdefault(rg,[]).append(it)
